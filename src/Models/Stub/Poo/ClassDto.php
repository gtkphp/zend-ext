...
class ClassDto extends BaseClassDto
{
    ...
    static public function create($codeGenerator, $renderer)
    {
        ...
        $dto->name = 'cairo_t'==$dto->name ? 'cairo_context_t' : $dto->name;
        $dto->name = GnomeClassifier::Prefix($dto->name, 'cairo');// remove 'cairo_'
        $dto->name = GnomeClassifier::Suffix($dto->name);// remove '_t'
        $dto->name = GnomeClassifier::SnakeToPascal($dto->name);
        ...
        foreach($classGenerator->getProperties() as $propertyGenerator) {
            ...
            $nullable = $propertyGenerator->getType()->nullable() ? '?' : '';
            $varDto->type = $nullable . $renderer->transfer('TypeDto.php', $propertyGenerator->getType())->name;
        ...
        foreach($fileGenerator->getFunctions() as $function) {
            if ($function->isMemoryManagement()
             || $function->isErrorManagement()
             || $function->isUserData()
             || $function->isStatic()
             || $function->isGetterSetter()) {
                continue;
            }
