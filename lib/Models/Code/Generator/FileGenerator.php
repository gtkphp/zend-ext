...
use Zend\Ext\Models\Code\DeclareStatement;
use Zend\Ext\Models\Code\Exception\InvalidArgumentException;
use Zend\Ext\Models\Code\Generator\Exception\ClassNotFoundException;
use Traversable;
...
class FileGenerator extends AbstractGenerator
{
    ...
    /**
     * @var EnumGenerator[]
     * @psalm-var array<string, EnumGenerator>
     */
    protected $enums = [];

    /**
     * @var AliasClassGenerator[]
     * @psalm-var array<string, AliasClassGenerator>
     */
    protected $aliasClass = [];
    
    /**
     * @var MethodGenerator[]
     * @psalm-var array<string, MethodGenerator>
     */
    protected $functions = [];
            ...
            switch (strtolower(str_replace(['.', '-', '_'], '', $name))) {
                ...
                case 'declares':
                    $fileGenerator->setDeclares(
                        array_map(function ($directive, $value) {return DeclareStatement::fromArray([$directive => $value]);},
                                    array_keys($value),
                                    $value)
                    );
                    break;
    ...
    public function setClass($class)
    {
        ...
        $class->setContainingFileGenerator($this);

        return $this;
    }
    /**
     * @param  array|string|EnumGenerator $class
     * @throws Exception\InvalidArgumentException
     * @return FileGenerator
     */
    public function setEnum($class)
    {
        if (is_array($class)) {
            $class = EnumGenerator::withConfig($class);
        } elseif (is_string($class)) {
            $class = EnumGenerator::withConfig(['name'=>$class]);
        } elseif (! $class instanceof EnumGenerator) {
            throw new Exception\InvalidArgumentException(sprintf(
                '%s is expecting either a string, array or an instance of %s\ClassGenerator',
                __METHOD__,
                __NAMESPACE__
    ...
    public function hasEnum($name = null)
    {
        if (empty($this->enums)) {
            return false;
        }
        if ($name !== null) {
            if (false === array_key_exists($name, $this->enums)) {
                return false;
            }
        }
    ...
    public function getEnum($name = null)
    {
        if ($name === null) {
            reset($this->enums);
            $class = current($this->enums);
            if (false === $class) {
                throw new ClassNotFoundException('No enum is set');
            }
    ...
    /**
     * @param  array|string|AliasClassGenerator $class
     * @throws Exception\InvalidArgumentException
     * @return FileGenerator
     */
    public function setAliasClass($class)
    {
        if (is_array($class)) {
            $class = AliasClassGenerator::fromArray($class);
        } elseif (is_string($class)) {
            $class = AliasClassGenerator::fromArray(['name'=>$class]);
        } elseif (! $class instanceof AliasClassGenerator) {
            throw new Exception\InvalidArgumentException(sprintf(
                '%s is expecting either a string, array or an instance of %s\ClassGenerator',
                __METHOD__,
                __NAMESPACE__
        ...
        // @todo check for dup here
        $className                 = $class->getName();
        $this->aliasClass[$className] = $class;
    ...
    public function hasAliasClass($name = null)
    {
        if (empty($this->aliasClass)) {
            return false;
        }
        if ($name !== null) {
            if (false === array_key_exists($name, $this->aliasClass)) {
                return false;
            }
        }
    ...
    public function getAliasClass($name = null)
    {
        if ($name === null) {
            reset($this->aliasClass);
            $class = current($this->aliasClass);
            if (false === $class) {
                throw new ClassNotFoundException('No enum is set');
            }
        ...
        if (false === array_key_exists($name, $this->aliasClass)) {
            throw new ClassNotFoundException(sprintf('aliasClass %s is not set', $name));
        }

        return $this->aliasClass[$name];
    }

    /**
     * @param  array[]|string[]|MethodGenerator[] $functions
     * @return FileGenerator
     */
     ...
     * @throws ClassNotFoundException Todo: Create MethodNotFoundException
     */
    public function getFunction($name = null)
    {
        ...
        if (false === array_key_exists($name, $this->functions)) {
            throw new ClassNotFoundException(sprintf('Class %s is not set', $name));
        }
    ...
    /**
     * @param  array|string|MethodGenerator $function
     * @throws Exception\InvalidArgumentException
     * @return FileGenerator
     */
    public function setFunction($function)
    {
        if (is_array($function)) {
            $function = MethodGenerator::fromArray($function);
        } elseif (is_string($function)) {
            $function = new MethodGenerator($function);
        } elseif (! $function instanceof MethodGenerator) {
            throw new Exception\InvalidArgumentException(sprintf(
                '%s is expecting either a string, array or an instance of %s\ClassGenerator',
                __METHOD__,
                __NAMESPACE__
