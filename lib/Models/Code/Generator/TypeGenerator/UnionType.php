...
final class UnionType
{
    ...
    public function toString(): string
    {
        ...
        return implode(
            '|',
            array_map(
                function ( $type): string  {return $type instanceof IntersectionType ? '(' . $type->toString() . ')' : $type->toString();}
                , $this->types
            )
        );
    }
    ...
    public function fullyQualifiedName(): string
    {
        ...
        return implode(
            '|',
            array_map(
                function ( $type): string  {return $type instanceof IntersectionType ? '(' . $type->fullyQualifiedName() . ')' : $type->fullyQualifiedName();}
                , $this->types
            )
        );
